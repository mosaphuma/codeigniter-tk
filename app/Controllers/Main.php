...
{   
    ...
    public function __construct()
    {
        ...
        $this->hr_empmast_model = new Staff;
        $this->it_pc_model = new IT_pc;
        $this->i_expanse_model = new I_expanse;
    ...
    public function index()
    {
        ...
        $this->data['countrysum']=$this->hr_country_model->countAll();
        $this->data['staffsum']=$this->hr_empmast_model->countAll();
        $this->data['pcsum']=$this->it_pc_model->countAllpc();
        $this->data['cctvsum']=$this->it_pc_model->countAllcctv();
        $this->data['fpsum']=$this->it_pc_model->countAllfp();
        $this->data['sumprice']=$this->i_expanse_model->sumprice();
...
public function staff(){
    ...
    if(!empty($this->request->getVar('search'))){
        ...
        $this->hr_empmast_model->where("lower(hr_empmast.`EMPCODE`) like '%{$search}%' or lower(hr_empmast.`FIRSTNAME`) like '%{$search}%'");
    }
    $this->data['total'] =  $this->hr_empmast_model->countAllResults();
    if(!empty($this->request->getVar('search'))){
        $search = strtolower($this->request->getVar('search'));
        $this->hr_empmast_model->where("lower(hr_empmast.`EMPCODE`) like '%{$search}%' or lower(hr_empmast.`FIRSTNAME`) like '%{$search}%'");    
    }
    $this->data['staff'] = $this->hr_empmast_model
                                ->select("hr_empmast.*, hr_empmast.EMPCODE,hr_empmast.FIRSTNAME")
                               
                                ->paginate($this->data['perPage']);
    $this->data['total_res'] = is_array($this->data['staff'])? count($this->data['staff']) : 0;
    $this->data['pager'] = $this->hr_empmast_model->pager;
    return view('pages/staffs/list', $this->data);
}
...
public function staff_add(){
    if($this->request->getMethod() == 'post'){
        ...
        $udata= [];
        $udata['EMPCODE'] = $code;
        $udata['FIRSTNAME'] = $name;
        $checkCode = $this->hr_empmast_model->where('EMPCODE',$code)->countAllResults();
        if($checkCode){
            $this->session->setFlashdata('error',"Staff Code Already Taken.");
        }else{
            $save = $this->hr_empmast_model->save($udata);
            if($save){
                $this->session->setFlashdata('main_success',"country Details has been updated successfully.");
                return redirect()->to('Main/staff/');
            }else{
                $this->session->setFlashdata('error',"staff Details has failed to update.");
            }
        }
    }

    $this->data['page_title']="Add New Staff";
    return view('pages/staffs/add', $this->data);
}
public function staff_edit($id=''){
    ...
    if($this->request->getMethod() == 'post'){
        ...
        $udata['EMPCODE'] = $code;
        $udata['FIRSTNAME'] = $name;
        $checkCode = $this->hr_empmast_model->where('EMPCODE',$code)->where("ID!= '{$id}'")->countAllResults();
        if($checkCode){
            $this->session->setFlashdata('error',"Staff Code Already Taken.");
        }else{
            $update = $this->hr_empmast_model->where('ID',$id)->set($udata)->update();
            if($update){
                $this->session->setFlashdata('success',"Staff Details has been updated successfully.");
                return redirect()->to('Main/staff_edit/'.$id);
            }else{
                $this->session->setFlashdata('error',"Staff Details has failed to update.");
            }
        }
    ...
    $this->data['staff'] = $this->hr_empmast_model->where("ID ='{$id}'")->first();
    return view('pages/staffs/edit', $this->data);
}

public function staff_delete($id=''){
    if(empty($id)){
            $this->session->setFlashdata('main_error',"Staff Deletion failed due to unknown ID.");
            return redirect()->to('Main/staff');
    }
    $delete = $this->hr_empmast_model->where('ID', $id)->delete();
    if($delete){
        $this->session->setFlashdata('main_success',"staff has been deleted successfully.");
    }else{
        $this->session->setFlashdata('main_error',"staff Deletion failed due to unknown ID.");
    }
    return redirect()->to('Main/staff');
